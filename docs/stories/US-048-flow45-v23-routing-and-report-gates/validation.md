...
| AC | Proof |
| --- | --- |
| Escrow deposit sets the milestone timeline and Expert start does not reset it. | `depositMilestoneEscrow_shouldSetInProgressStartedAt` (assertNotNull cm.inProgressStartedAt after deposit); `startMilestone_shouldNotResetInProgressStartedAt` (assertEquals depositTime, cm.inProgressStartedAt after start). |
...
| The acknowledgement carries no structured feedback or revision semantics. | `acknowledgeProgressReport_shouldSetAcknowledgedAndNotify` verifies ACKNOWLEDGED state, acknowledgedBy, acknowledgedAt; structured feedback fields removed from entity and no longer writable; `ProgressReportFeedbackRequest` DTO deleted; `notifyProgressReportFeedbackRecorded` notification method removed. |
