...
expanded panels, contact affordances, and rating display. Profile entities keep
persisted KYB/KYC data in `business_profiles` and `expert_profiles`;
account-owned display/contact fields are enriched from `account`, and rating
summary is enriched from `reviews`, into transient response fields.

## Business Profile Reads

The following APIs return `BusinessProfileEntity` with `fullName`, `email`, and
`phone` populated from the owning account, plus `averageRating` calculated from
`AVG(reviews.rating)` where `reviews.reviewee_id = business.accountId`:
...
`phone`, and `title` populated from the owning account where applicable, plus
`averageRating` calculated from `AVG(reviews.rating)` where
`reviews.reviewee_id = expert.accountId`:
...
response fields sourced from existing `account` and `reviews` rows.
